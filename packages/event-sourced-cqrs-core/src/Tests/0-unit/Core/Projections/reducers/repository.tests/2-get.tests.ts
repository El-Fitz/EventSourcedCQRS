...
export const testSuites: TestSuite[] = [
	(() => {
		...
		const implementation = (title: string) => (parameters?: TestSuiteParameters) => (_expectedResult?: TestSuiteExpectedResult) => (platform: Platform.PlatformInterface) => (test: TestInterface<unknown>) => {
			test(title, async t => {
				const repository = platform.Projections.Reducers.Repository;
	...
	(() => {
		...
		const expectedResults = parameters?.projections.reducers[0].reducer as Core.Projections.Reducers.Reducer;
